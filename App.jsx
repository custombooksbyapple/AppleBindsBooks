...
const PRODUCTS = [
  {
    ...
    videoUrls: [
      { url: "https://www.youtube.com/embed/H4xcKzTA9z0", vertical: false },
      { url: "https://www.youtube.com/embed/VktBqVfQhcs", vertical: true },
    ],
  },
  {
    id: 1,
  ...
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .video-embed-wrap.vertical {
      width: 60%; max-width: 320px; padding-top: 177.78%;
      margin-left: auto; margin-right: auto;
    }
    .video-embed-wrap:last-child { margin-bottom: 0; }
    .video-embed-wrap.gallery-video {
      margin-bottom: 0;
    }
    .video-embed-wrap.gallery-video.vertical {
...
function getYoutubeId(embedUrl) {
  const match = embedUrl && embedUrl.match(/embed\/([a-zA-Z0-9_-]+)/);
...
function ProductPage({ product, onBack, onContact }) {
  ...
  const media = [
    ...
    ...(product.videoUrls || []).map((v) => ({
      type: "video",
      url: v.url,
      vertical: v.vertical,
      thumb: `https://img.youtube.com/vi/${getYoutubeId(v.url)}/mqdefault.jpg`,
    })),
  ];
      ...
      <div className="detail-layout">
        ...
        <div className="detail-gallery">
          <div className="detail-main-img-wrap">
            {active.type === "image" ? (
              <img src={active.src} alt={product.title} className="detail-main-img" />
            ) : (
              <div className={`video-embed-wrap gallery-video ${active.vertical ? "vertical" : ""}`}>
                <iframe
                  src={active.url}
            ...
            {media.length > 1 && (
              <>
                <button
                  ...
                  aria-label="Previous"
                  onClick={() => setActiveImg((activeImg - 1 + media.length) % media.length)}
                >‹</button>
                <button
                  type="button"
                  className="detail-nav-btn next"
                  aria-label="Next"
                  onClick={() => setActiveImg((activeImg + 1) % media.length)}
                >›</button>
              </>
            )}
          </div>
          <div className="detail-thumbs">
            {media.map((m, i) =>
              m.type === "image" ? (
                <img
                  key={i} src={m.src} alt="" className={`detail-thumb ${i === activeImg ? "active" : ""}`}
                  onClick={() => setActiveImg(i)}
                />
              ) : (
                <div
                  key={i}
                  className={`detail-thumb video-thumb ${i === activeImg ? "active" : ""}`}
